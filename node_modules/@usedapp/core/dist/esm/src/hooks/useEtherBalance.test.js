...
import { renderDAppHook, setupTestingConfig, SECOND_TEST_CHAIN_ID } from '../testing';
...
describe('useEtherBalance', () => {
    ...
    it('explicitly mainnet', async () => {
        const { result, waitForCurrent } = await renderDAppHook(() => useEtherBalance(receiver, { chainId: Mainnet.chainId }), { config });
        await waitForCurrent((val) => val !== undefined);
        expect(result.error).to.be.undefined;
        expect(result.current).to.eq(100);
    });
    it('explicitly specified chain id', async () => {
        const { result, waitForCurrent } = await renderDAppHook(() => useEtherBalance(receiver, { chainId: SECOND_TEST_CHAIN_ID }), { config });
        await waitForCurrent((val) => val !== undefined);
        expect(result.error).to.be.undefined;
        expect(result.current).to.eq(200);
    });
