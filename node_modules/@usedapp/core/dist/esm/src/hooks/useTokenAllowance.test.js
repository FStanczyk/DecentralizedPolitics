...
describe('useTokenAllowance', () => {
    ...
    it('multichain calls return correct initial balances', async () => {
        await testMultiChainUseTokenAllowance(token, deployer.address, spender.address, ChainId.Localhost, MOCK_TOKEN_INITIAL_BALANCE.toString());
        await testMultiChainUseTokenAllowance(secondToken, secondDeployer.address, secondSpender.address, SECOND_TEST_CHAIN_ID, SECOND_MOCK_TOKEN_INITIAL_BALANCE.toString());
    });
    const testMultiChainUseTokenAllowance = async (contract, user, spenderUser, chainId, allowance) => {
        ...
        const { result, waitForCurrent } = await renderWeb3Hook(() => useTokenAllowance(contract.address, user, spenderUser, { chainId }), {
            mockProvider: {
